...
D3D12_VERTEX_BUFFER_VIEW gunVBV = {};
D3D12_INDEX_BUFFER_VIEW gunIBV = {};
MeshData meshData;
void ProcessNode(aiNode* node, const aiScene* scene,
    ID3D12GraphicsCommandList* commandList, ID3D12Device* device);
void CreateBuffersFromMesh(MeshData& meshData,
    ID3D12GraphicsCommandList* commandList, ID3D12Device* device);
ID3D12Resource* CreateBuffer(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, void* data, size_t dataSize,
    D3D12_RESOURCE_FLAGS flags, D3D12_RESOURCE_STATES initState, ID3D12Resource** uploadBuffer);
...
void SsaoApp::Draw(const GameTimer& gt)
{
    ...
    model *= XMMatrixRotationX(30);
    //model *= XMMatrixRotationY(-90);
    //model = XMMatrixScaling(1,1,1);
    XMStoreFloat4x4(&oc.World, XMMatrixTranspose(model));
    res->CopyData(0, oc);
    mCommandList->SetPipelineState(mPSOs["opaque"].Get());
    
    mCommandList->SetGraphicsRootConstantBufferView(0, res->Resource()->GetGPUVirtualAddress());
    mCommandList->IASetVertexBuffers(0, 1, &gunVBV);
    mCommandList->IASetIndexBuffer(&gunIBV);
    mCommandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    mCommandList->DrawIndexedInstanced(meshData.indices.size(), 1, 0, 0, 0);
    ...
    mCommandList->IASetVertexBuffers(0, 1, &m_SkyBoxVBV);
    mCommandList->SetGraphicsRootConstantBufferView(0, res->Resource()->GetGPUVirtualAddress());
    mCommandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
...
void SsaoApp::LoadTextures()
{
    ...
    std::vector<std::string> texFilenames =
    {
        ...
        "D:/SmileEngine/Assets/Textures/EnvirMap/brown_photostudio_06_2k.hdr"
    };
	...
	for(int i = 0; i < (int)texNames.size(); ++i)
	{
        ...
        unsigned char* imageData = stbi_load(texMap->Filename.c_str(), &width, &height, &channels, STBI_rgb_alpha);
        if (!imageData) {
            // 错误处理
            assert(1);
        }
...
void SsaoApp::BuildShapeGeometry()
{
    ...
    const aiScene* scene = importer.ReadFile("D:/SmileEngine/Assets/Models/Cerberus_LP.FBX",
    ...
    ProcessNode(scene->mRootNode, scene, mCommandList.Get(), md3dDevice.Get());
    
    D3D12_RESOURCE_DESC vertexBufferDesc = CD3DX12_RESOURCE_DESC::Buffer(sizeof(SkyBoxVertex) * skyBoxVertices.size());
    md3dDevice->CreateCommittedResource(
        ...
        D3D12_RESOURCE_STATE_COMMON,
        nullptr,
        IID_PPV_ARGS(&m_SkyBox)
    );
    D3D12_RANGE range{0,0};
    
    ThrowIfFailed(m_SkyBox->Map(0, nullptr, reinterpret_cast<void**>(&data)));
    memcpy(data, skyBoxVertices.data(), sizeof(SkyBoxVertex) * skyBoxVertices.size());

    m_SkyBoxVBV.BufferLocation = m_SkyBox->GetGPUVirtualAddress();
    m_SkyBoxVBV.SizeInBytes = sizeof(SkyBoxVertex) * skyBoxVertices.size();
    ...
    ID3D12GraphicsCommandList* commandList, ID3D12Device* device)
{
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; i++)
    {
        ...
        meshData = ProcessMesh(mesh, scene, commandList, device);
        std::wstring buffer(std::to_wstring(meshData.vertices.size()));
        OutputDebugString(L"\n--------------\n");
        OutputDebugString(buffer.c_str()); OutputDebugString(L"\n");
        CreateBuffersFromMesh(meshData, commandList, device);
    }

    // 递归处理子节点
    for (unsigned int i = 0; i < node->mNumChildren; i++)
    {
        ProcessNode(node->mChildren[i], scene, commandList, device);
    }
}
    ...
    ID3D12GraphicsCommandList* commandList, ID3D12Device* device)
{
    ...
    return meshData;
    // 现在你可以使用 vertices 和 indices 数据创建 DirectX 12 的顶点缓冲区和索引缓冲区
}

// Function to create DirectX 12 buffers from mesh data
void CreateBuffersFromMesh(MeshData& meshData, 
    ID3D12GraphicsCommandList* commandList, ID3D12Device* device) {
    ...
    size_t vertexBufferSize = sizeof(Vertex) * meshData.vertices.size();

    vertexBuffer = CreateBuffer(device, commandList, meshData.vertices.data(), vertexBufferSize,
        D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &vertexUploadBuffer);

    // Create index buffer
    ID3D12Resource* indexBuffer;
    ID3D12Resource* indexUploadBuffer;

    size_t indexBufferSize = sizeof(uint32_t) * meshData.indices.size();

    indexBuffer = CreateBuffer(device, commandList, meshData.indices.data(), indexBufferSize,
        D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_INDEX_BUFFER, &indexUploadBuffer);

    // Create vertex buffer view
    
    gunVBV.BufferLocation = vertexBuffer->GetGPUVirtualAddress();
    gunVBV.StrideInBytes = sizeof(Vertex);
    gunVBV.SizeInBytes = static_cast<UINT>(vertexBufferSize);

    // Create index buffer view
    gunIBV.BufferLocation = indexBuffer->GetGPUVirtualAddress();
    gunIBV.Format = DXGI_FORMAT_R32_UINT;
    gunIBV.SizeInBytes = static_cast<UINT>(indexBufferSize);

    // Here, you can store vertexBufferView and indexBufferView for rendering later
}

// Helper function to create a DirectX 12 buffer (uploading the data)
ID3D12Resource* CreateBuffer(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, void* data, size_t dataSize,
    D3D12_RESOURCE_FLAGS flags, D3D12_RESOURCE_STATES initState, ID3D12Resource** uploadBuffer) {
    ...
    CD3DX12_RESOURCE_DESC bufferDesc = CD3DX12_RESOURCE_DESC::Buffer(dataSize, flags);
    ...
    // Create an upload buffer for transferring data
    ...
    UpdateSubresources(commandList, buffer, *uploadBuffer, 0, 0, 1, &subresourceData);

    // Set resource barrier to transition the buffer to its intended state
    CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(buffer, D3D12_RESOURCE_STATE_COPY_DEST, initState);
